...
BIN_WIDTH = 0.5  # Offset from the middle of bin mass to the threshold edges


class AsciiFiletypes(enum.IntEnum):
    """
...
class BaseIntensityMatrix(pymsBaseClass, TimeListMixin, MassListMixin, IntensityArrayMixin, GetIndexTimeMixin):
    ...
    def __init__(
        ...
        self._mass_list = numpy.array(mass_list)

        self._mass_lookup = {}
        for index, mass_entry in enumerate(self._mass_list):
            self._mass_lookup[mass_entry] = index
    ...
    def __eq__(self, other) -> bool:
        ...
        if isinstance(other, self.__class__):
            return (
                self.time_list == other.time_list and self.mass_list == other.mass_list and
                numpy.array_equal(self.intensity_array, other.intensity_array)
            )
    ...
    def get_ic_at_index(self, ix: int) -> IonChromatogram:
        ...
        ic_ia = self._intensity_array[:, ix]  # ia = [intensities[ix] for intensities in self._intensity_array]
    ...
    def get_index_of_mass(self, mass: float) -> int:
        ...
        best = self._max_mass
        ix = 0
        for idx, test_mass in enumerate(self._mass_list):
            tmp = abs(test_mass - mass)
            if tmp < best:
                best = tmp
                ix = idx
        return ix

    def get_mass_lookup(self, mass: int) -> int:
        """
        Return the index of the existing mass. Does not compute closest, mass must be in existing list
        :param mass: Mass generated from earlier ic
        """
        return self._mass_lookup[mass]
    ...
    def crop_mass(self, mass_min: float, mass_max: float):
        ...
        self._mass_list = numpy.array(self._mass_list)  # In case cached values are using a list
        # pre build mass_list and list of indecies
        mass_mapping = numpy.logical_and(mass_min <= self._mass_list, self._mass_list <= mass_max)
        self._mass_list = self._mass_list[mass_mapping]
        self._intensity_array = numpy.array(self._intensity_array)[:, mass_mapping]

        self._min_mass = min(self._mass_list)
        self._max_mass = max(self._mass_list)
...
class IntensityMatrix(BaseIntensityMatrix):
    ...
    def get_ic_internal(self, mass: int) -> IonChromatogram:
        ix = self.get_mass_lookup(mass)
        return self.get_ic_at_index(ix)
...
def _fill_bins(
    ...
    for scan in data.scan_list:  # use the alias, not the copy (Luke)
        ...
        masses = scan._mass_list  # use the alias, not the copy (Luke)
        intensities = scan._intensity_list  # use the alias, not the copy (Luke)
