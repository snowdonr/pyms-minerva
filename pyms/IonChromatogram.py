...
class IonChromatogram(pymsBaseClass, TimeListMixin, IntensityArrayMixin, GetIndexTimeMixin):
    ...
    def __eq__(self, other: Any) -> bool:
        ...
        if isinstance(other, self.__class__):
            return (
                self.time_list == other.time_list and
                all(numpy.equal(self.intensity_array, other.intensity_array)) and self.mass == other.mass
            )
...
#         td_array = numpy.array(td_list)
        td_array = numpy.diff(self._time_list)  # numpy.array(self._time_list[1:]) - numpy.array(self._time_list[:-1])
